...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            InlineKeyboardButton('🤖 Updates', url='https://t.me/STMbOTsUPPORTgROUP'),
...
async def auto_filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if imdb and imdb.get('poster'):
            await message.reply_photo(photo=imdb.get('poster'), caption=f"<b>🎬 Film/Series : <a href={imdb['url']}>{imdb.get('title')}</a>\n🌟 Rating : <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> | IMDB\n📆 Release : <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a>\n🎭 Genres : {imdb.get('genres')}\n©{message.chat.title}🍿</b>", reply_markup=InlineKeyboardMarkup(btn))
        elif imdb:
            await message.reply_text(f"<b>🎬 Film/Series : <a href={imdb['url']}>{imdb.get('title')}</a>\n🌟 Rating : <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> | IMDB\n📆 Release : <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a>\n🎭 Genres : {imdb.get('genres')}\n©{message.chat.title}🍿</b>", reply_markup=InlineKeyboardMarkup(btn))
        else:
            await message.reply_photo(photo='https://telegra.ph/file/ff0b04abaf31f135f3ea0.jpg',caption=f"<b>🎬 Film/Series : {search}\n🌟 Rating : {random.choice(TRATING)}\n📆 Release : None\n🎭 Genres : {random.choice(TGENRES)}\n©{message.chat.title}🍿</b>", reply_markup=InlineKeyboardMarkup(btn))
